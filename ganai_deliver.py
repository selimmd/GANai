""" GANai v1.0
	- Add additional GANai D loss for G training
	- Add GAN discriminator to the model
...
Model = collections.namedtuple("Model", "outputs, predict_real, predict_fake, gan_predict_real, gan_predict_fake, discrim_loss, gen_loss_GAN, gen_loss_L1, train")
...
# load training data for generator
...
# load training data for discriminator
...
def create_model_gen_train(inputs, targets):
	...
	with tf.name_scope("discriminator_loss"):
		...
		#gan_discrim_loss = tf.reduce_mean(-(tf.log(gan_predict_real + EPS) + tf.log(1 - gan_predict_real + EPS)))
